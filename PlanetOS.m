...
Begin["`Private`"]
  ...
  Pointdata[datasets_:"hycom_glbu0.08_91.2_global_0.08d",lon_,lat_,opts___?OptionQ]:=Module[{dataseturl},
	If[opts=={},
	dataseturl= apirooturl<>datasets<>"/point?lon="<>ToString[lon]<>"&lat="<>ToString[lat]<>"&apikey="<>$apikey;,
	dataseturl= apirooturl<>datasets<>"/point?lon="<>ToString[lon]<>"&lat="<>ToString[lat]<>StringJoin[StringJoin[{"&",ToString[#[[1]]],"=",ToString[#[[2]]]}]&/@Flatten[opts]]<>"&apikey="<>$apikey ;
	];
	Import[dataseturl,"json"]
	]
   Areadata[datasets_:"hycom_glbu0.08_91.2_global_0.08d",polygon_,opts___?OptionQ]:=Module[{dataseturl},
	If[opts=={},
	dataseturl= apirooturl<>datasets<>"/area?polygon="<>ToString[polygon]<>"&apikey="<>$apikey;,      
	dataseturl= apirooturl<>datasets<>"/area?polygon="<>ToString[polygon]<>StringJoin[StringJoin[{"&",ToString[#[[1]]],"=",ToString[#[[2]]]}]&/@Flatten[opts]]<>"&apikey="<>$apikey ;
	];
	Import[dataseturl,"json"]
	]
   Stations[datasets_:"noaa_ndbc_stdmet_stations"]:=Module[{datasetssurl},
	datasetssurl = apirooturl<>datasets<>"/stations?apikey="<>$apikey ;
	Import[datasetssurl ,"json"]
	]
   Stationdata[datasets_:"noaa_ndbc_stdmet_stations",station_,opts___?OptionQ]:=Module[{dataseturl},
	If[opts=={},
	dataseturl= apirooturl<>datasets<>"/stations/"<>station<>"?apikey="<>$apikey;,
 dataseturl= apirooturl<>datasets<>"/stations/"<>station<>"?apikey="<>$apikey<>StringJoin[StringJoin[{"&",ToString[#[[1]]],"=",ToString[#[[2]]]}]&/@Flatten[opts]] ;
	];
	Import[dataseturl,"json"]
	]
End[]
